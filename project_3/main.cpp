...
int main() {
    ...
    hangman game;
    //load all words from dictionary.txt into the vector
    // Keep playing the game until the user decides otherwise

    while (true) {
        //prompt user for length of word, reprompting if invalid word length
        //delete all words from dictionary that are not of desired length
        cout << "How long of a word would you like?" << endl;
        while (true) {
            cin >> wordLength;
            if (game.dic.count(wordLength) == 1) {
                break;
             }
            cerr << "Sorry " << wordLength << " is not a valid length" << endl;
        }

        //Prompt user for how many guesses they would like
        int num_guesses;
        ...
        while (true) {
            ...
            if (num_guesses > 0)
                break;
            cerr << "Number of guesses must be greater than 0." << endl;
        }
        ...
        cout << "Show running total of words? (y/n)" << endl;
        char c;
        cin >> c;
        if (c == 'n')
            seeWords = false;

        //start game with values user entered above
